	...
	public void sendGet() throws IOException, InterruptedException {
		...
		while (true) {
			synchronized (lock) {
				...
				URI telegramGetUpdateUrl = URI.create(TELEGRAM_BASE_URL + "/" + TelegramClient.telegramToken +
					"/getUpdates" +
					"?offset=-1");
...
//				}
	...
	private void handleAuthCommand(int randomClientID, Integer chatId) throws IOException, InterruptedException {
		// TODO вынести сборку URI в TelegramClient
		URI url = URI.create(TELEGRAM_BASE_URL + "/" + TelegramClient.telegramToken + "/sendMessage?chat_id=" + chatId + "&text="
			+ URLEncoder.encode(StravaClient.STRAVA_OAUTH_ADDRESS + "authorize?client_id="+StravaClient.stravaClientId +"&state=" + randomClientID + "&response_type=code&redirect_uri=http://localhost:8080" +
